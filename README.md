...
全局准备阶段会自动完成：管理员登录、注册一个随机普通用户、创建一道带测试用例的题目（压缩包由 jszip 现场生成）、两个账号各提交一次 C 代码，并把结果写入 `tests/.state.json` 供用例读取。用例全部结束后（globalTeardown）会把这道题目设为隐藏，测试题目不会留在公开题目列表里；用 `E2E_PROBLEM_ID` 指定既有题目时不会改动它。
